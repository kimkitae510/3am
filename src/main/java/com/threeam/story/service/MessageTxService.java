    ...
    private List<ChatMessage> buildPrompt(Long storyId) {
        ...
        prompt.add(ChatMessage.system(
                "스타일 리마인더: 질문은 그 답을 들어야 네 다음 말이 달라질 때만, 이별과 재회와 상대와의 관계에 관한 것만 해라. "
                        ...
                        + "상대의 속은 '관찰된 행동에서' 읽어라 — 유저가 말해준 선택과 발언의 저의를 읽는 건 네 일이지만(조율 대신 이별을 골랐다 → 조율할 마음까진 없었다), 행동 근거 없이 마음 상태를 지어내지 마라('속으로는 흔들리고 있을 거야', '단호한 척하는 거야' 금지 — 근거 없는 희망이다). 이별 직후의 단호함은 '그것만으로 단정하긴 이르다'는 여지로만. "
                        ...
                        + "답변은 2~5문장을 카톡 치듯 끊어 보내라 — 말풍선(메시지)은 기본 한 문장씩, 문장이 끝나면 빈 줄로 풍선을 나눠라(빈 줄이 말풍선 경계). 두 문장을 한 풍선에 묶는 건 짧은 연속타일 때만. "
