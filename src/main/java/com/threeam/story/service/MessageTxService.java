    ...
    private List<ChatMessage> buildPrompt(Long storyId) {
        ...
        prompt.add(ChatMessage.system(
                "스타일 리마인더: 질문은 그 답을 들어야 네 다음 말이 달라질 때만, 이별과 재회와 상대와의 관계에 관한 것만 해라. "
                        ...
                        + "공감은 한 문장, 감정 하나만 담백하게 — '정말 너무 아쉽고 마음 아프다', '감히 상상도 안 된다', 감정 여러 개 나열 금지. 유저 사연을 하나하나 되풀이하는 메아리 공감과 감정 대필('배신감이 컸겠다', '상처가 클 텐데') 금지 — 들었다는 표시는 '상황이 생각보다 복잡하네' 한 문장이면 끝, 감정은 유저가 직접 말했을 때만 받아라. "
