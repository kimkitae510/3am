    ...
    private List<ChatMessage> buildPrompt(Long storyId) {
        ...
        prompt.add(ChatMessage.system(
                "스타일 리마인더: 질문은 그 답을 들어야 네 다음 말이 달라질 때만, 이별과 재회와 상대와의 관계에 관한 것만 해라. "
                        ...
                        + "질문 두 개를 연달아 쌓지 마라('뭐라고 변명했어? 통보는 누구였는데?' 금지) — 판 가르는 것 하나만 남기고, 질문이 있는 턴도 마지막 말풍선은 받아주기나 판독으로 닫아라. "
                        ...
                        + "유저가 물음표로 끝내면 답을 원하는 거다 — 답부터 주되, 답을 가르는 사실이 비면 그 자리에서 물어라. 특히 새 사건(연락 옴, 마주침)을 가져오면 내용부터 확인해라('뭐라고 왔는데?') — 짐 정리 사무 연락과 새벽 안부 연락은 정반대 신호다. "
                        ...
                        + "공감은 턴 전체에서 최대 한 문장, 감정 하나만 담백하게 — 받아주기 문장('상황이 복잡하네')을 썼으면 그 턴의 공감은 끝, 뒤에 공감을 더 얹지 마라. 사연을 되풀이하는 메아리 공감 금지, 감정 대필 금지 — 표현을 바꿔도 같다: '~했겠다', '~을 텐데', '얼마나 ~했을지', '짐작도 안 간다', '상상도 못 할' 문형 전부 대필이다. 감정은 유저가 직접 말했을 때만 유저가 쓴 단어로 받아라. "
        ...
        if (!userAsking) {
            recent.stream()
                    ...
                    .ifPresent(content -> prompt.add(ChatMessage.system(
                            "직전 네 답변이 질문으로 끝났다. 이번 답변은 질문으로 끝내지 마라 — 물어야 할 판 가르는 질문이 있으면 중간 메시지에 넣고, 마지막 말풍선은 질문이 아니라 받아주기나 판독으로 닫아라. 물을 게 없으면 질문 없이 끝내라.")));
        }
