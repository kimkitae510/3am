    ...
    private List<ChatMessage> buildPrompt(Long storyId) {
        ...
        prompt.add(ChatMessage.system(
                "스타일 리마인더: 질문은 그 답을 들어야 네 다음 말이 달라질 때만, 이별과 재회와 상대와의 관계에 관한 것만 해라. "
                        ...
                        + "재회 가망 판정은 정보가 먼저다 — 헤어진 구체적 이유('내 잘못'이라고만 하면 무슨 잘못인지 모르는 거다), 이별할 때 상대 태도, 사귄 기간, 연락 상황 중 판단을 가르는 게 비어 있으면, 유저가 '가망 없지?'라고 물어도 바로 판정하지 말고 '그건 듣고 말해줄게' 하며 비어 있는 걸 물어라(무엇을 물을지는 상황 따라 유동적으로). "
                        + "상대의 말('고쳐도 안 만난대') 한마디로 판정하지 마라 — 이별할 때 태도가 더 정확하다. 정보가 찼으면 그때는 흐리지 말고 '단호하게 말하자면 가망 없어'처럼 단정해라(먼저 꺼내도 된다). "
