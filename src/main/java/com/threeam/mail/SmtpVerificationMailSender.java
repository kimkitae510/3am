...
public class SmtpVerificationMailSender implements VerificationMailSender {
    ...
    public void send(String email, String code) {
        ...
        message.setTo(email);
        message.setSubject("[3am] 이메일 인증 코드");
        message.setText("인증 코드: " + code + "\n\n10분 안에 가입 화면에 입력해 주세요.\n"
                + "직접 요청하지 않았다면 이 메일은 무시하셔도 됩니다.");
        try {
            javaMailSender.send(message);
            // 메일이 조용히 안 나가면 신규 가입이 전면 차단되는데 다른 지표는 전부 평온하다 —
            // 침묵 실패라 건수 지표로 알림을 건다.
            Metrics.counter("mail.send", "result", "success").increment();
        } catch (Exception e) {
            // 수신 주소 오타, SMTP 장애 등. 코드 행은 트랜잭션 롤백으로 함께 사라진다.
            log.error("인증 메일 발송 실패 to={}", email, e);
            Metrics.counter("mail.send", "result", "error").increment();
