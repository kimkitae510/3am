    ...
    public CompletableFuture<ReunionDiagnosis> diagnose(List<String> knownFactLines,
        ...
        prompt.add(ChatMessage.system(
                "출력 직전 마지막 점검 — 아래에 걸리면 고치고 출력해라: "
                        + "1) 요인 판정의 주어: 결론이 상대의 마음('돌아오고 싶을', '회복하고 "
                        + "싶을')을 말하면 근거도 상대 쪽 사실(상대가 한 행동, 들인 것, 말한 "
                        + "것)이어야 한다. 유저가 받은 것, 느낀 것, 기억하는 것을 상대의 "
                        + "유인으로 잇고 있으면 그 근거는 빼라. 신뢰가 무너지고 실망하고 속은 "
                        + "쪽이 유저면 빼는 것도 같은 검사다 — "
                        ...
                        + "7) verdict가 POSSIBLE인데 breakupType이 비어 있으면, 유형을 비우는 "
                        + "판(유저 쪽 사정으로 끝났거나 상대의 잘못으로 유저가 끝냈지만 상대 "
                        + "마음은 안 떠난 판)인지 확인해라 — 그 판이면 유저통보 3단 점프가 "
                        + "반드시 있어야 하고, 아니면 유형부터 다시 판정해라. "
